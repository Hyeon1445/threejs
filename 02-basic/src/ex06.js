...
export default function example() {
  ...
  function draw() {
    //const time = clock.getElapsedTime(); // 실행 시간으로부터 총 경과시간
    const delta = clock.getDelta(); // 시간 간격 (이전 frame , 이번 frame 시간차), getElapsedTime이랑 같이 쓰면 안됨.

    //mesh.rotation.y += 0.1; // 각도는 radian사용
    //mesh.rotation.y += THREE.MathUtils.degToRad(1); // radian을 degree로 변환해주는 utils
    mesh.rotation.y += 2 * delta; // time을 활용 -> device별 시간차 보정
