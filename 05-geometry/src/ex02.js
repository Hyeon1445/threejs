...
export default function example() {
  ...
  for (let i = 0; i < positionArray.length; i += 3) {
    // 정점(vertex) 한개의 x,y,z 좌표를 랜덤으로 조절
    positionArray[i] += (Math.random() - 0.5) * 0.2; // x
    positionArray[i + 1] += (Math.random() - 0.5) * 0.2; // y
    positionArray[i + 2] += (Math.random() - 0.5) * 0.2; // z

    randomArray[i] = (Math.random() - 0.5) * 0.2; // x
    randomArray[i + 1] = (Math.random() - 0.5) * 0.2; // y
    randomArray[i + 2] = (Math.random() - 0.5) * 0.2; // z
  }
