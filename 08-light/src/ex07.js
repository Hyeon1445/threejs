...
export default function example() {
  ...
  const rectAreaLight = new THREE.RectAreaLight("orange", 10, 2, 2);
  rectAreaLight.position.y = 2;
  rectAreaLight.position.z = 4;
  scene.add(rectAreaLight);

  const rectAreaLightHelper = new RectAreaLightHelper(rectAreaLight); // light의 위치를 확인 가능.
  ...
  gui.add(rectAreaLight.position, "x", -5, 5).name("light x");
  gui.add(rectAreaLight.position, "y", -5, 5).name("light y");
  gui.add(rectAreaLight.position, "z", -5, 5).name("light z");
